      ...
      <Fragment>
          ...
          {thumbs1.map((item, index) => {
            return (
              <Colxx xxs="6" lg="2" md="4" key={index}>
                <NavLink
                  to="#"
                  onClick={() => this.onThumbClick(index, 1)}
                  location={{}}
                >
                  <img
          ...
          {thumbs2.map((item, index) => {
            return (
              <Colxx xxs="6" lg="3" md="3" key={index}>
                <NavLink
                  to="#"
                  onClick={() => this.onThumbClick(index, 2)}
                  location={{}}
                >
                  <img
                    className="img-fluid border-radius"
        ...
        {isOpen && (
          <Lightbox
            ...
            nextSrc={
              this.getGallery()[(photoIndex + 1) % this.getGallery().length]
            }
            prevSrc={
              this.getGallery()[
                (photoIndex + this.getGallery().length - 1) %
