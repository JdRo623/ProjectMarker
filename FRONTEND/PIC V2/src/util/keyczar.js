...
import keyczar_util from "./keyczar_util";
...
keyczar.create = function (type, purpose, options) {
  ...
  if (type === keyczar.TYPE_RSA_PRIVATE) {
    ...
    keyString = keyczar_util._rsaPrivateKeyToKeyczarJson(
  ...
  if (
    !(
      purpose === keyczar.PURPOSE_DECRYPT_ENCRYPT ||
      purpose === keyczar.PURPOSE_SIGN_VERIFY
    )
  ) {
...
function _exportPublicKey(key) {
  ...
  if (
    !(
      t === keyczar.TYPE_RSA_PRIVATE &&
      (p === keyczar.PURPOSE_DECRYPT_ENCRYPT ||
        p === keyczar.PURPOSE_SIGN_VERIFY)
    )
  ) {
...
var PBE_DECRYPT_FAILED_MESSAGE =
  "AES decryption failed (password incorrect or data is corrupt?)";
...
function _makeKeyczar(data, password) {
  ...
  if (t === keyczar.TYPE_RSA_PRIVATE) {
    ...
    instance.exportPublicKey = function () {
      return _exportPublicKey(instance);
    };
...
keyczar.createSessionCrypter = function (key, sessionMaterial) {
  if (
    key.metadata.type !== keyczar.TYPE_RSA_PRIVATE &&
    key.metadata.type !== keyczar.TYPE_RSA_PUBLIC
  ) {
    throw new Error(
      "Invalid key type for SessionCrypter: " + key.metadata.type
    );
  }
...
keyczar.encryptWithSession = function (key, message) {
  ...
  var packed = keyczar_util._packByteStrings([
    crypter.rawSessionMaterial,
    rawEncrypted,
  ]);
