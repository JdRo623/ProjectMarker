                        ...
                        try {
                                ...
                                colum.eachCell(function (cell, rowNumber) {
                                    ...
                                    if (cell.value) {
                                        if (rowNumber == 1) {
                                            ...
                                            } else if (cell.value.includes("Subdirección") || cell.value.includes("Dirección") || cell.value.includes("División")) {
                            ...
                            CursosHandler.insertMany(actividades, (err, actividades) => {
                                ...
                                SubdireccionHandler.insertMany(subGrupos, (err, roles) => {
                                    if (err) return res.status(500).send({ estado: 'Error', message: 'Error en la petición', data: Object.assign({ "error": err }) });
                                    if (!roles) return res.status(200).send({ estado: 'Error', message: 'No fue posible registrar las Subdirecciones', data: Object.assign({}) });
                                    ExcepcionesHandler.insertMany(excepciones, (err, excepcionesGuardadas) => {
                                        if (err) return res.status(500).send({ estado: 'Error', message: 'Error en la petición', data: Object.assign({ "error": err }) });
                                        if (!excepcionesGuardadas) return res.status(200).send({ estado: 'Error', message: 'No fue posible registrar las excepciones', data: Object.assign({}) });
                                        CoordinacionHandler.insertMany(coordinaciones, (err, coordinacionesGuardadas) => {
                                            if (err) return res.status(500).send({ estado: 'Error', message: 'Error en la petición', data: Object.assign({ "error": err }) });
                                            if (!coordinacionesGuardadas) return res.status(200).send({ estado: 'Error', message: 'No fue posible registrar las coordinaciones', data: Object.assign({}) });
                                            SeccionalHandler.insertMany(seccionales, (err, sec) => {
                                                ...
                                                CargosHandler.insertMany(cargosGuardarDef, (err, car) => {
                                                    if (err) return res.status(500).send({ estado: 'Error', message: 'Error en la petición', data: Object.assign({ "error": err }) });
                                                    if (!car) return res.status(200).send({ estado: 'Error', message: 'No fue posible registrar los roles', data: Object.assign({}) });
                                    ...
                                    SeccionalHandler.find({}, (err, seccionales) => {
                                        RolHandler.find({}, (err, subObt) => {
                                            ...
                                            cargosOnt.forEach(cargoObt => {
                                                ...
                                                columnaCarrier = 3
                                                hojasCount++;
                                                const worksheet = workbook.addWorksheet(cargoObt.nombre
                                                    .replace("de", "")
                                                ...
                                                columnas.forEach(fila => {
                                                    worksheet.getRow(fila[0] + 1).getCell(1).value = fila[0]
                                                    worksheet.getRow(fila[0] + 1).getCell(2).value = fila[1]
                                                })
...
const columnas = [
    ...
    [2, "	Principios, estructura del Estado colombiano y los derechos fundamentales 	"],
    ...
    [7, "	Políticas de servicio al cliente en entidades públicas, estrategias, ética de la prestación y servicio base 	"],
    [8, "	Principios éticos en el ejercicio del servicio 	"],
    [9, "	Comportamientos personales, laborales y organizacionales en la norma y la  ética	"],
    ...
    [16, "	Casos prácticos sobre solución de conflictos y arbitraje en contratación pública	"],
    ...
    [21, "	Ofimática avanzada para análisis de datos, procesamiento de texto y presentación de informes 	"],
    [22, "	Arquitectura de Información y Organización de Metadatos y Ontologías para la Gestión de Datos y Estadísticas.	"],
    ...
    [27, "	Capacitación de autores a partir de una estrategia pedagógica para el desarrollo de contenidos en procesos misionales y de apoyo de la DIAN	"],
    [28, "	Capacitación de tutores en estrategia pedagógica para entornos virtuales	"],
    ...
    [35, "	Técnicas de comunicación: Entrevista, presentaciones orales y escritas	"],
    ...
    [43, "	Análisis de entornos y administración de la interdependencia organizacional	"],
    ...
    [58, "	Diplomado en régimen probatorio con énfasis tributario, aduanero y cambiario, a partir de una estrategia pedagógica y de contenidos.	"],
    ...
    [63, "	Programa en Clasificación Arancelaria. Módulo 1: Clasificación arancelaria aplicada	"],
    [64, "	Programa en Clasificación Arancelaria. Módulo 2: clasificación arancelaria de profundización por sectores de la Industria	"],
    [65, "	Control y auditoría de operaciones de comercio exterior por sectores especializados	"],
    ...
    [70, "	Validación de metodologías analíticas y buenas prácticas de laboratorio.	"],
    ...
    [78, "	Daño Antijurídico: Estudio Teórico Práctico de Líneas JurisprudencialesTACI	"],
    [79, "	Daño Antijurídico: Técnica legislativa TACI	"],
    [80, "	Habilidades de Oratoria Jurídica	"],
    [81, "	Probatorio Penal: Medios de Prueba.	"],
    [82, "	Jurisprudencia y aspectos prácticos de las pruebas en el sistema penal acusatorio 	"],
    [83, "	Análisis del Riesgo Jurídico en materia penal	"],
    [84, "	Introducción a contabilidad y finanzas para juristas y otras profesiones	"],
    [85, "	Contabilidad para la interpretación de peritazgo técnico	"],
    [86, "	Finanzas para la interpretación de peritazgo técnico	"],
    [87, "	Curso de recaudación - Módulo 2 Control de entidades autorizadas a recaudar	"],
    [88, "	Curso de recaudación - Módulo 3 Devoluciones y/o compensaciones	"],
    [89, "	Introducción a la contabilidad para entidades recaudadoras y otras entidades públicas	"],
    [90, "	Contabilidad para entidades recaudadoras y otras entidades públicas – Activos y Pasivos	"],
    [91, "	Contabilidad para entidades recaudadoras y otras entidades públicas – Información presentada	"],
    [92, "	Análisis Financiero	"],
    [93, "	Programa en Conocimientos Académicos en Administración de Cartera - Modulo 1 Función Cartera y Modulo 2 Facilidades de pago	"],
    [94, "	Programa en Conocimientos Académicos en Administración de Cartera - Módulo 3 Cobro Persuasivo, Módulo 4 Medidas Cautelares, Módulo 5 Cobro Coactivo y Módulo 8 Jurisprudencia y Doctrina	"],
    [95, "	Programa en Conocimientos Académicos en Administración de Cartera - Módulo 6 Procesos especiales.	"],
    [96, "	Información financiera para diferentes tipos de entidades y su análisis	"],
    [97, "	Introducción normativa al proceso de comercialización	"],
    [98, "	Normativa de las modalidades de disposición de mercancías ADA en la DIAN 	"],
    ...
    [102, "	Planeación financiera y comercial: Modelos de toma de decisión y gestión por indicadores	"],
    ...
    [108, "	Metodologías para la presentación de proyectos en contexto internacional	"],
    ...
    [131, "	Taller de armonización del catálogo presupuestal y contable - Caso DIAN	"],
    [132, "	Taller en presupuesto, contabilidad, tesorería y control de gestión - Casos DIAN	"],
    [133, "	Función del Control Interno	"],
    [134, "	Ejecución de la Auditoría Interna	"],
    [135, "	Evaluación del sistema de control interno	"],
    [136, "	Entrenamiento para certificaciones de auditoría en sistemas de gestión	"],
    ...
    [151, "	Principios constitucionales (Control de Convencionalidad en el Derecho Disciplinario)	"],
    [152, "	Planeación Estratégica del servicio, Gerencia de Clientes, Gestión de Brechas 	"],
    ...
    [160, "	Liderazgo, proyección y seguimiento de resultados de investigaciones de usuario	"],
    ...
    [179, "	Diseño de servicios digitales (Design thinking y experiencia de usuario)	"],
